...
class FaceClassifier():
    ...
    def get_face_image(self, frame, box):
        ...
        crop_right = min(box_right + box_width, img_width - 1) #Image size
    ...
    def detect_faces(self, frame):
        boxes = self.locate_faces(frame) #small box
        if len(boxes) == 0:
            return []
    ...
    def draw_name(self, frame, face):
        ...
        crop_right = min(right + box_width, img_width - 1) #Image size
        ...
        if height > (bottom - top) // 3:
            height = (bottom - top) // 3
        #빈자리
        size = [crop_top, crop_bottom, crop_left, crop_right] #Desire 좌표
        # draw name
        #cv2.rectangle(frame, (left, bottom + 35), (right, bottom), (0, 0, 255), cv2.FILLED)
        font = cv2.FONT_HERSHEY_DUPLEX
        cv2.putText(frame, face.name, (left + 6, bottom + 30), font, 1.0,
                    (255, 255, 255), 1)
        return face.name, size
...
if __name__ == '__main__':
    ...
    while running:
        ...
        if args.display or args.capture:
            for face in faces:
                name, size=fc.draw_name(frame, face)
                print("\n",frame_id,name,":",size) #Index 및 좌표 출력
                numbers = re.findall("\d+", name)
                if not numbers:
                    numbers=['00']
                b = np.array([numbers[0], frame_id])
                array3 = np.concatenate([b, size])
                a = np.append(a, np.array([array3]), axis=0)
